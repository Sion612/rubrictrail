...
test("a two-page PDF rejects page 3 before project creation", async ({ page }) => {
  ...
  await expect(page.getByTestId("criterion-name-2")).toHaveValue("Manual verification criterion");
  await page.getByTestId("criterion-weight-0").focus();
  await expect(page.getByTestId("criterion-weight-0")).toBeFocused();
  await page.getByTestId("criterion-weight-0").fill("45");
  await expect(page.getByTestId("criterion-weight-0")).toHaveValue("45");
  await page.getByTestId("criterion-weight-1").focus();
  await expect(page.getByTestId("criterion-weight-1")).toBeFocused();
  await page.getByTestId("criterion-weight-1").fill("45");
  await expect(page.getByTestId("criterion-weight-1")).toHaveValue("45");
  await page.getByTestId("criterion-weight-2").focus();
  await expect(page.getByTestId("criterion-weight-2")).toBeFocused();
  await page.getByTestId("criterion-weight-2").fill("10");
  await expect(page.getByTestId("criterion-weight-2")).toHaveValue("10");
