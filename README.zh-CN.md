...
> 当前公开 Demo 已部署并独立验证 v0.8.0 多作业工作区对应的精确产品合并版本。
> 最终 `v0.8.0` 标签与 GitHub Release 仍需等待本次仅文档的发布证据变更通过自己的
> main、Pages 与公网门禁，因此此处不提前声称正式 Release 已发布。
...
1. 当前公开 v0.8.0 Demo 首页显示 **我的作业**，并可新建虚构的 LumaLane 示例。
...
产品合并版本已通过 PR exact-head、合并后 main、exact-SHA Pages 与独立公网验证；
最终标签和 GitHub Release 在本次仅文档的证据变更完成自身门禁后发布。完整远端、
公网与尚未验证边界见[验证报告](./docs/TEST_REPORT.md)。
