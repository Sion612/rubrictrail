...
> 当前公开 Demo 仍是已经通过完整 CI 与 Pages 门禁的 v0.7.1。此分支中的
> v0.8.0 多作业工作区仍是发布候选；在 PR、合并后 main、Pages 和公网验证全部
> 完成前，不应把它描述为已经上线。
...
1. 当前公开 v0.7.1 Demo 可直接选择虚构的 LumaLane 示例；v0.8.0 完成发布门禁后，
   首页将先显示 **我的作业**。
2. 依次查看 **作业说明 → 评分标准 → 计划 → 自检 → 进度**。
3. v0.8.0 发布后，可使用醒目的 **新建作业** 上传可信的 TXT、DOCX、文本型 PDF、
...
完整候选/远端/公网证据边界见[验证报告](./docs/TEST_REPORT.md)。
